...
int setup_client_socket_fd(sockaddr_in client_address);
...
void setup_fd_set(const port_t kServerPort,
  ...
  client_socket_fd= setup_client_socket_fd(client_address);
...
int setup_client_socket_fd(sockaddr_in client_address) {
  ...
  int opt = 1;
  HANDLE_CALL_ERRORS(setsockopt(client_socket_fd, SOL_SOCKET, SO_REUSEADDR, (char *)&opt, sizeof(opt)),
                     EXIT_SOCK_ERROR);
